...
func choose_random_location():
	var x = min(GameData.roll(object_owner.get_map_position().x+5, object_owner.get_map_position().x-5), (GameData.MAP_SIZE.x-1))
	var y = min(GameData.roll(object_owner.get_map_position().y+5, object_owner.get_map_position().y-5), (GameData.MAP_SIZE.y-1))
	var pos = Vector2(x,y)
...
func throw_rock_at_player():
	GameData.map.spawn_rock("Goblin", GameData.map.map_to_world(object_owner.get_map_position()))
	number_of_rocks -= 1
	wind_down()

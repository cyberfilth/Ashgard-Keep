...
func read_book():
	...
	GameData.broadcast("You read a passage from the book...", GameData.COLOUR_LIGHT_BLUE)
	GameData.broadcast(passage, GameData.COLOUR_YELLOW)
	boost_stat()

func boost_stat():
	var boost = GameData.roll(0,2)
	if boost == 0:
		GameData.broadcast("Your new knowledge inspires you. Your attack strength has increased +1", GameData.COLOUR_GREEN)
		GameData.player.fighter.attack += 1
	if boost == 1:
		GameData.broadcast("A new determination fills you. Your defence ability has increased +1", GameData.COLOUR_GREEN)
